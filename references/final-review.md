...
- I1：旁白能否在脱离画面时仍构成“问题 → 机制 → 证据 → 限制 → 结论”的完整论证；
- I2：每个关键数字是否显示来源、时间范围和统计口径；示意、估算和未核验内容是否明确标注；
- I3：图表是否从 JSON 数据源生成，且其字幕、旁白与图表出现的时段一致；
- I4：对比矩阵是否以同粒度、可比较的维度组织，未把不同口径拼成结论；
- I5：BGM 与 SFX 是否为旁白让位，而非把论证剪成产品宣传片。
