...
### Q12. 商业洞察片的数据口径必须可见
规则：商业洞察片中每个驱动判断的数字、趋势或比较都必须带可读的来源、时间范围和口径；不确定、估算或示意数据必须显式说明。为了画面简洁删掉来源，等同于删掉论证条件。`SourceNote` 是默认落位，图表没有来源注记不得进入终检。
...
商业洞察例外：旁白是主轨，BGM 只作低能量铺底，SFX 仅标记证据、转折或结论的关键瞬间；连续 whoosh、强鼓点和 UI 反馈音会把纪录片论证降格为产品宣传。
...
### C1. 按片种决定文案顺序；纯动画段落也配解说

软件介绍片可以在画面锁定后按最终镜头重写文案。商业洞察片必须反转顺序：先写可独立成立的旁白论证，再以每段旁白安排图表、证据与结论画面；旁白、字幕与音频分段从同一个 `NarrationSegment` 生成，避免错位。
