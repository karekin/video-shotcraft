...
商业洞察片不以“功能清单”作为唯一输入。开始分镜前先固定论证链：问题、机制、证据、反例或限制、结论；并为每个关键数据记录来源、时间范围、统计口径和不确定性。数据写入 `template/src/video-styles/business-insight/data/*.json`，旁白分段写入对应 `Main.tsx` 的 `NARRATION` 表。换报告应换 JSON、旁白和分镜，而不是改图表组件。
...
商业洞察片将本阶段替换为**数据准备**：核验数据源、整理 JSON、写清来源/口径/ caveat，并为每张图表安排与旁白一致的 `SourceNote`。真实网页截图只在它本身构成证据时使用，而不是默认视觉装饰。
...
商业洞察片先放入分段旁白与字幕同源轨道，再垫极低音量的 BGM，最后只给转折或结论加克制 SFX。没有旁白音频时，`NarrationTrack` 仍应保留字幕和段落锚点，不能让 BGM 或 SFX 伪装成叙事。
